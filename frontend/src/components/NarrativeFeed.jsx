  ...
  return (
    ...
    >
      <span className="uppercase tracking-[0.25em] font-bold"
        style={{ color: s.color, fontFamily: "Cinzel, serif", fontSize: "0.6rem" }}>
        {s.label}
      </span>
...
function NarrativeEntry({ entry, isStreaming = false }) {
  return (
    <div
      className="mb-10"
      style={{ borderLeft: "2px solid var(--border-warm)", paddingLeft: "1.5rem", position: "relative" }}
    >
      {/* Entry marker — gold pulse when streaming */}
      <div
        style={{
          position: "absolute",
          left: -5,
          ...
          width: 8,
          height: 8,
          background: isStreaming ? "var(--gold-mid)" : "var(--blood-mid)",
          border: `1px solid ${isStreaming ? "var(--gold-text)" : "var(--blood-bright)"}`,
      ...
      {/* Player input */}
      <div className="text-xs italic mb-3 flex items-start gap-1.5"
      ...
      {/* Outcome badge — show immediately */}
      <OutcomeBadge
      ...
      {/* Prose — streaming or final */}
      <div
        className="leading-[1.85] mb-4"
        style={{
          fontFamily: "'IM Fell English', Georgia, serif",
          fontSize: "1.05rem",
          color: isStreaming ? "var(--parchment-mid)" : "var(--parchment)",
          whiteSpace: "pre-wrap",
      ...
      >
        {entry.narrative || (
          isStreaming && (
            <span className="italic" style={{ color: "var(--mist)", fontSize: "0.9rem" }}>
              The realm stirs…
            </span>
          )
        )}
      ...
      {/* Downstream changes — only on committed entries */}
      {!isStreaming && Object.keys(entry.downstream_changes || {}).length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-2" style={{ borderTop: "1px solid var(--border-dim)" }}>
          {Object.entries(entry.downstream_changes).map(([node, state]) => (
            <span key={node} className="text-xs px-2 py-0.5"
              style={{
...
export default function NarrativeFeed({ entries, streamingEntry, isLoading }) {
  const bottomRef = useRef(null);

  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [entries, streamingEntry, isLoading]);

  const isEmpty = entries.length === 0 && !streamingEntry && !isLoading;
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ padding: "2rem 2.5rem" }}>
      {isEmpty && (
        <div className="flex flex-col items-center justify-center h-full" style={{ paddingTop: "4rem" }}>
          ...
          <p className="text-sm italic text-center"
            style={{ color: "var(--parchment-dim)", fontFamily: "'IM Fell English', Georgia, serif", maxWidth: 300 }}>
      ...
      {/* Task 5 — streaming in-progress entry */}
      {streamingEntry && (
        <NarrativeEntry entry={streamingEntry} isStreaming />
      )}

      {/* Fallback spinner for non-streaming path */}
      {isLoading && !streamingEntry && (
        <div className="mb-8" style={{ borderLeft: "2px solid var(--border)", paddingLeft: "1.5rem" }}>
          <p className="text-sm italic animate-pulse"
            style={{ color: "var(--mist)", fontFamily: "'IM Fell English', Georgia, serif" }}>
